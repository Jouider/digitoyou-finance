...
@section('page-title', 'Modifier la Distribution')

@section('content')
<div class="max-w-4xl mx-auto">
    <div class="bg-white rounded-lg shadow-md p-8">
        <form action="{{ route('profit-distributions.update', $profitDistribution) }}" method="POST" id="distributionForm">
            @csrf
            @method('PUT')
                ...
                <select name="project_id" id="project_id" required
                    ...
                    @foreach($projects as $project)
                        <option value="{{ $project->id }}" 
                            ...
                            {{ old('project_id', $profitDistribution->project_id) == $project->id ? 'selected' : '' }}>
            ...
            <div class="mb-6">
                ...
                <input type="number" name="total_profit" id="total_profit" step="0.01" required
                    ...
                    value="{{ old('total_profit', $profitDistribution->total_profit) }}"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                    ...
                    <input type="number" name="agency_percentage" id="agency_percentage" step="0.01" required
                        ...
                        value="{{ old('agency_percentage', $profitDistribution->agency_percentage) }}"
                        oninput="calculateShares()">
                ...
                <div>
                    ...
                    <input type="number" name="sadaqah_percentage" id="sadaqah_percentage" step="0.01" required
                        ...
                        value="{{ old('sadaqah_percentage', $profitDistribution->sadaqah_percentage) }}"
                        oninput="calculateShares()">
            ...
            <div class="mb-6">
                ...
                <input type="date" name="distribution_date" id="distribution_date" required
                    ...
                    value="{{ old('distribution_date', $profitDistribution->distribution_date->format('Y-m-d')) }}">
            ...
            <div class="mb-8">
                ...
                <textarea name="notes" id="notes" rows="4"
                    ...
                    placeholder="Notes supplémentaires sur cette distribution...">{{ old('notes', $profitDistribution->notes) }}</textarea>
                @error('notes')
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
    ...
    <div class="mt-6 bg-gray-50 rounded-lg p-4">
        <div class="flex items-center text-sm text-gray-600">
            ...
            <span>Créée le {{ $profitDistribution->created_at->format('d/m/Y à H:i') }}</span>
            @if($profitDistribution->updated_at != $profitDistribution->created_at)
                <span class="mx-2">•</span>
                <span>Modifiée le {{ $profitDistribution->updated_at->format('d/m/Y à H:i') }}</span>
            @endif
        </div>
    </div>
</div>
